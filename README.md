...
### ⚙️ Backend
   ...
   10. Created ALB in EC2 console
      - Added listener for HTTP and HTTPS
        1. Requested new ACM certificate for api.banksie.app
        2. Added A record to Route 53 for api.banksie.app (will need time to propagate)
      ...
      - Created target group for ecs tasks (For an IP but do not add any targets)
        1. Add health check path as /health
        2. In Flask app added route for /health to return code 200
